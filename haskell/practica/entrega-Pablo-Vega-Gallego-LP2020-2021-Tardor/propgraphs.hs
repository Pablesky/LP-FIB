---autor: Pablo Vega Gallego
--curs: Tardor Q1 2021-2022
--Assigantura: LP

--Part principal de programa on recopilem les dades d'on està situat el graf i les seves dades.
main :: IO()
main = do
  putStrLn $ "Fitxer per defecte (pulsa 0) o vols llegir un fitxer customitzat (pulsa 1)"
  numero <- getLine
...
--Bucle per a poder realitzar les operacions sobre el graf de manera interactiva.
bucle :: Graf -> IO()
bucle graf = do
...
--Creem un ABC que farem servir per a realitzar gran part de les consultes sobre el graf.
...
--Tipus de dada que representa una aresta i els dos vertexs que uneix, amb les seves instancies definidies
...
--Tipus de dades que relaciona un punt(aresta o vertex) i una propietat amb un valor
...
--Funcions auxiliars per tal de conseguir Props
...
--          rho.pg   lambda.pg  sigam.pg tvalue.pg
...
--Afegim label a aresta i/o vertex
...
--Obtenir les k primeres propietats de un graf (vertex)
...
--Obtenir les k primeres propietats de un graf (aresta)
...
--Et diu si la aresta (v1, v2) te el seu inici al vertex desitjat
esInici :: String -> (String, String) -> Bool
esInici inicio (v1, _) = v1 == inicio

--Et diu si la aresta (v1, v2) acaba al punt desitjat
esSolucio :: String -> (String, String) -> Bool
esSolucio final (_, v2) = v2 == final

--Transforma una aresta a una llista de vertexs als que va destinat
dePairALlista :: [(String, String)] -> [String]
dePairALlista [] = []
dePairALlista ((_,v1):resta) = v1:dePairALlista resta

--Implentacio BFS per dir si es pot anar d'un vertex a un altre
...
--No he sapigut implementar els k salts :(
--He implementat una funcio que donat un graf pots anar del vertex inici al vertex
--dessitjat, en un numero de salts indefinit.
